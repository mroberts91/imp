...
// or a negative nice all need it: the systemd ordering), then the mount
// sandbox (privateTmp + filesystem, M10-b) and the bounding drops (both
// need capabilities the identity drop clears), then the capability-aware
// identity drop, then noNewPrivileges (late, so it cannot interfere with
// the privileged steps), then umask, then execve. Returns only on error.
func apply(p *Payload) error {
	...
	privateTmp := p.PrivateTmp != nil && *p.PrivateTmp
	if privateTmp || p.Filesystem != nil {
		if err := setupMounts(privateTmp, p.Filesystem); err != nil {
			return err
		}
	}
	var id *identity
